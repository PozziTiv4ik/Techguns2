...
/** Source daylight behavior, including the persistent Techguns-spawner origin exemption. */
public abstract class BurningUndeadNpc extends ArmedNpc {
    ...
    @Override public void aiStep() {
        if(!level().isClientSide() && isAlive() && !hasSpawnerOrigin() && level().environmentAttributes().getValue(EnvironmentAttributes.MONSTERS_BURN,position())) {
