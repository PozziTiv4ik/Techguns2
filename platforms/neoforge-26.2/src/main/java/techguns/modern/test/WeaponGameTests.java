    ...
    public static void registerTests(RegisterGameTestsEvent event) {
        ...
        FUNCTIONS.getEntries().forEach(function -> event.registerTest(function.getId(),
                new FunctionGameTestInstance(function.getKey(),
                        new TestData<>(environment, TGContent.id("weapon_test"), function.getId().getPath().startsWith("charging_") || function.getId().getPath().startsWith("fabricator_") ? 400 : function.getId().getPath().startsWith("reaction_") ? 800 : function.getId().getPath().startsWith("blast_") ? 1200 :
