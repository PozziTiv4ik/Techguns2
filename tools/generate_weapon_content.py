...
from legacy_zombie_soldier import generate_zombie_soldier_content, zombie_soldier_translations
...
from legacy_policeman import generate_policeman_content, policeman_translations
from legacy_police_station import generate_police_station_content
...
from legacy_building import generate_building_content, building_translations
...
''')
    ...
    for name in BUG_SOUNDS: selected_sounds[name] = {"sounds":sounds_data[name]["sounds"]}
    ...
    for sound, value in selected_sounds.items():
        ...
        for lang in languages:
            ...
            if sound in BUG_SOUNDS: translated[lang][subtitle] = "Alien Bug" if lang == "en_us" else "Инопланетный жук"
            ...
            if sound in FORTIFICATION_SOUNDS: translated[lang][subtitle] = 'Bunker door moves' if lang == 'en_us' else 'Движется бункерная дверь'
    ...
    for lang, values in translated.items():
        ...
        values.update(zombie_soldier_translations(lang))
...
'''
    ...
    for path, value in [entry for domain in (generate_ore_content(), generate_fluid_content(), generate_chemical_content(), generate_reaction_content(), generate_radiation_content(), generate_fabricator_content(), generate_charging_content(), generate_rocket_content(), generate_npc_content(), generate_cyber_content(), generate_armor_content(), generate_repair_content(), generate_camo_content(), generate_grinder_content(), generate_zombie_soldier_content(), generate_rural_content(), generate_skeleton_content(), generate_bandit_content(), generate_chainsaw_content(), generate_psycho_content(), generate_spawner_content(), generate_army_content(), generate_commando_content(), generate_policeman_content(), generate_police_station_content(), generate_location_content(), generate_ghastling_content(), generate_alienbug_content(), generate_spike_content(), generate_cluster_content(), generate_drill_content(), generate_building_content(), generate_fortification_content(), generate_camonet_content(), generate_meteor_content(), generate_bugnest_content(), generate_nether_castle_content(), generate_medium_altar_content(), generate_ghast_spawner_content()) for entry in domain.items()]:
        if path in files:
