    ...
    def test_armor_tags_merge_without_dropping_combat_armor(self):
        ...
        for part,tag in [('helmet','head'),('chestplate','chest'),('leggings','leg'),('boots','foot')]:
            for path in (tag+'_armor','enchantable/'+tag+'_armor'):
                ...
                self.assertEqual(set(values),{'techguns:hazmat_'+part,'techguns:t2_combat_'+part,'techguns:t1_combat_'+part})
        self.assertEqual(len(json.loads(files[RESOURCES+'data/minecraft/tags/item/enchantable/durability.json'])['values']),12)
